...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mContent != null) {
            ...
            // if the layout's height is wrap_content, set the layout' height to
            // be the same with the content's height
    ...
    public boolean dispatchTouchEventSupper(MotionEvent e) {
        boolean back = false;
        try {
            back = super.dispatchTouchEvent(e);
        } catch (Exception ex) {
            PtrCLog.e(LOG_TAG, ex.getMessage());
        }
        return back;
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mForceBackWhenComplete) {
                    ...
                    // when footer is showing and status is completed, do not abort
                    // scroller.
                    boolean isFooter = !mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if
                    // the
                    // footer
                    // is
                    // showing
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // }
                /**
                 * zp 解决内嵌带横向滑动控件的横向滑动问题，替代了原来上面的475到481行
                 */
                ...
                boolean canMoveUp = mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if
                // the
                // header
                // is
                // showing

                boolean canMoveDown = mFooterView != null && !mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if
                // the
                // footer
                // is
                // showing
                ...
                boolean canFooterMoveUp = mPtrHandler != null && mFooterView != null // The footer view could be null, so
                        // need double check
    ...
    /**
     * Scroll back to to if is not under touch When forceback is true and footer
     * is completed, scroll back either
     */
